...
//
// A prior version of this component shipped without the onFinalize() below
// and reliably soft-locked the screen on some devices: the zoomed overlay is
// pointerEvents="none" by design (see pinchZoom.tsx) and relied entirely on
// onTouchesUp/onTouchesCancelled to reset `hidden`/`zoomingRef` — whenever
// RNGH's native touch lifecycle didn't deliver one of those (never fully
// root-caused, no device farm to reproduce it against), the underlying photo
// stayed hidden with no way to tap it again. onFinalize() is the one RNGH
// hook that's guaranteed to fire exactly once whenever a gesture handler
// deactivates for ANY reason — success, fail, or the OS yanking the gesture
// away (app backgrounding, an incoming call, etc.) — so it's used here purely
// as an unconditional cleanup net, independent of which touch events did or
// didn't arrive.
export function PinchZoomPhoto({
  ...
  const manual = Gesture.Manual()
    ...
    // Unconditional cleanup net — see the note above. Idempotent with the
    // resets above (they already flip zoomingRef before this ever runs).
    .onFinalize(() => {
      if (zoomingRef.current) {
        zoomingRef.current = false;
        endZoom();
