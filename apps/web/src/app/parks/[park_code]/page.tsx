      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 22px 24px", position: "relative" }}>
        {latestVisit ? (
          <div>
            ...
            {/* Notes */}
            {latestVisit.notes ? (
              <div style={{ fontSize: 15, color: inkPaper, lineHeight: 1.65, marginBottom: 16 }}>
                {latestVisit.notes}
              </div>
            ) : (
            ...
            {latestVisit.photos && latestVisit.photos.length > 0 && (
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 9.5, letterSpacing: "1.4px", color: "var(--ink-mute)", textTransform: "uppercase", marginBottom: 8, fontWeight: 600 }}>
            ...
            {earlierVisits.length > 0 && (
              <div style={{ paddingTop: 16, borderTop: `0.5px dashed ${hairlinePaper}` }}>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 9.5, letterSpacing: "1.4px", color: "var(--ink-mute)", textTransform: "uppercase", marginBottom: 10, fontWeight: 600 }}>
                  EARLIER VISITS
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {earlierVisits.map((v, i) => {
                    ...
                    return (
                      <div key={i} style={{ background: "rgba(0,0,0,0.04)", borderRadius: 8, padding: "10px 12px", borderLeft: "2px solid var(--primary)" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 3 }}>
                          <div style={{ fontFamily: "var(--font-mono)", fontSize: 9.5, letterSpacing: "1px", color: "var(--ink-mute)", fontWeight: 600 }}>
                            {d.full.toUpperCase()}
                          </div>
                          <button
                            onClick={() => onEdit(v)}
                            style={{ background: "transparent", border: "none", padding: "2px 4px", cursor: "pointer", color: "var(--ink-mute)", flexShrink: 0, display: "flex", alignItems: "center" }}
                          >
                            <PenLine size={11} strokeWidth={2} />
                          </button>
                        </div>
                        {v.notes && <div style={{ fontSize: 12.5, color: inkPaper, lineHeight: 1.45 }}>{v.notes}</div>}
                        {v.title && !v.notes && <div style={{ fontSize: 12.5, color: inkPaper }}>{v.title}</div>}
                      </div>
                    );
                  })}
                </div>
  ...
  return (
    ...
    >
      <div style={{
        ...
        <TreePine size={30} style={{ color: "var(--primary)" }} strokeWidth={1.6} />
      </div>

      <div>
        <div style={{ fontWeight: 800, fontSize: 20, color: inkPaper, letterSpacing: -0.3, lineHeight: 1.2, marginBottom: 8 }}>
          Been to {parkName}?
        </div>
        <div style={{ fontSize: 13.5, color: "rgba(58,46,28,0.65)", lineHeight: 1.6 }}>
          Log your visit, write journal entries, earn badges, and track all 63 national parks on your personal map.
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%" }}>
        <Link href="/sign-up" style={{ textDecoration: "none" }}>
          <button style={{
            ...
            fontSize: 14, fontWeight: 700, color: "#FFFBF1", cursor: "pointer",
          }}>
            Create free account
          </button>
        </Link>
        <Link href="/sign-in" style={{ textDecoration: "none" }}>
          <button style={{
            ...
            border: "0.5px solid rgba(58,46,28,0.22)",
            borderRadius: 10, padding: "11px 20px",
            fontSize: 14, fontWeight: 600, color: inkPaper, cursor: "pointer",
          }}>
            Sign in
          </button>
        </Link>
      ...
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "1.2px", color: "rgba(58,46,28,0.4)", textTransform: "uppercase", fontWeight: 600 }}>
...
function ParkInfoContent({
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, overflowY: "auto", background: "var(--bg)" }}>
      {/* Breadcrumb */}
      <div style={{ padding: "18px 32px 0", display: "flex", alignItems: "center", gap: 6 }}>
        <Link
          href="/parks"
          style={{ display: "flex", alignItems: "center", gap: 4, color: "var(--ink-mute)", textDecoration: "none", fontSize: 12.5, fontWeight: 500 }}
        >
          <ChevronLeft size={14} strokeWidth={2.2} /> Parks
        </Link>
        <span style={{ color: "var(--ink-mute)", fontSize: 12 }}>›</span>
        <span style={{ fontSize: 12.5, color: "var(--ink)", fontWeight: 700 }}>{park.name}</span>
      </div>
      ...
      <div style={{ padding: "14px 32px 0" }}>
        <div style={{ position: "relative", borderRadius: 14, overflow: "hidden", height: 360 }}>
          ...
          <div style={{ position: "absolute", bottom: 18, left: 22, right: 22, color: "#FFFBF1", pointerEvents: "none" }}>
            <div style={{ marginBottom: 6 }}>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "rgba(255,251,241,0.92)", letterSpacing: "1.4px", textTransform: "uppercase", textShadow: "0 1px 3px rgba(0,0,0,0.6)" }}>
                {stateName}
              </div>
            </div>
            <div style={{ fontWeight: 800, fontSize: 44, letterSpacing: -1, lineHeight: 1 }}>{park.name}</div>
          </div>
        </div>
      </div>
      ...
      {isSignedIn && (
        <div style={{ padding: "0 32px 24px" }}>
          <div style={{ background: "var(--surface)", border: "0.5px solid var(--hairline)", borderRadius: 12, padding: "14px 0", display: "grid", gridTemplateColumns: "repeat(4, 1fr)" }}>
            ...
            <StatTile label="Visits" value={visits.filter((v) => !v.is_bucket_list && v.visited_date).length.toString()} unit="trips" border />
            <StatTile label="Photos" value={visits.flatMap((v) => v.photos ?? []).length.toString()} unit="saved" border />
          </div>
        </div>
      )}
      ...
      <div style={{ padding: "0 32px 32px", display: "flex", gap: 10 }}>
        {isSignedIn ? (
          <>
            {status !== "visited" ? (
              <DesktopButton primary onClick={onLogVisit}>
                <Plus size={14} strokeWidth={2.4} /> Log a visit
              </DesktopButton>
            ) : (
              <DesktopButton onClick={onLogVisit}>
          ...
          <Link href="/sign-up" style={{ textDecoration: "none" }}>
            <DesktopButton primary>
              <Plus size={14} strokeWidth={2.4} /> Track this park
            </DesktopButton>
          </Link>
        )}
      </div>
...
export default function ParkDetailPage({
  ...
  // Fetch all parks for the mini-map
  useEffect(() => {
    fetch("/api/parks").then((r) => r.ok ? r.json() : []).then(setAllParks).catch(() => {});
  }, []);

  // Fetch park data for everyone (public)
  useEffect(() => {
    fetch(`/api/parks/${park_code}`)
      .then((r) => r.ok ? r.json() : null)
  ...
  // Fetch visits only when signed in
  useEffect(() => {
    if (!isSignedIn) return;
    fetch("/api/visits")
      .then((r) => r.ok ? r.json() : [])
      .then((allVisits) => {
        const parkVisits = (allVisits as VisitData[]).filter((v) => v.park_code === park_code);
  ...
  const openEditVisit = (v: VisitData) => {
    setLogVisitDraft({
      parkCode: park_code,
      dates: { start: v.visited_date ? new Date(v.visited_date) : null, end: null },
      title: v.title ?? "",
      notes: v.notes ?? "",
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 14, color: "#FFFBF1" }}>
              Track your visit to {park.name}
            </div>
            <div style={{ fontSize: 12.5, color: "rgba(255,251,241,0.75)", marginTop: 2 }}>
              Log visits, earn badges, and explore all 63 national parks on ParkQuest.
            </div>
          </div>
