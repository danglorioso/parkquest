/**
 * Adds NPS "National Historic Site" units to the parks table — third
 * designation expansion after National Historical Parks and National
 * Monuments (see add-historical-parks.mjs, add-national-monuments.mjs).
 * No schema step needed — designation/is_national_park already exist.
 *
...
const historicSites = npsParks.filter((p) => p.designation === "National Historic Site");
console.log(`${historicSites.length} are National Historic Site`);

const dbParks = await sql`SELECT park_code FROM parks`;
const existingCodes = new Set(dbParks.map((r) => r.park_code));

const toInsert = historicSites.filter((p) => !existingCodes.has(p.parkCode));
const skippedCollisions = historicSites.filter((p) => existingCodes.has(p.parkCode));
